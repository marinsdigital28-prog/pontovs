...
export default function EmployeesPanel({
  ...
  async function saveProfile() {
    ...
    setSaving(true);
    setMessage('');
    // NÃO envia matrícula, workDays nem horários
  ...
  return (
    <section className="admin-two-col employees-cadastro-layout">
      <div className="card">
        <div className="section-heading">
          <div>
            ...
            <p className="small-muted">
              Contatos e documentos para comprovante por e-mail/SMS. Matrícula, dias e horários ficam na aba Turnos.
            </p>
          </div>
        ...
        <input
          className="input"
          placeholder="Buscar nome, matrícula, CPF, telefone, e-mail ou cidade"
          value={search}
        ...
        <div className="employee-list" style={{ marginTop: 12 }}>
          {filtered.map((employee) => {
            ...
            return (
              <div
                className={`employee-row ${active ? 'employee-row-active' : ''}`}
                key={employee.id}
                role="button"
                tabIndex={0}
                onClick={() => openEmployee(employee)}
                onKeyDown={(e) => e.key === 'Enter' && openEmployee(employee)}
                style={{ cursor: 'pointer' }}
              >
                <div>
                  <strong>{employee.name}</strong>
                  <div className="small-muted">
                    {employee.employeeNumber || 'Sem matrícula'} · {employee.jobTitle || 'Cargo —'}
                ...
                <div className="row-actions" onClick={(e) => e.stopPropagation()}>
                  <span className={employee.active ? 'status-pill ok' : 'status-pill off'}>
                    {employee.active ? 'Ativo' : 'Inativo'}
                  </span>
                  <span className={c.hasPhone && c.hasEmail ? 'status-pill ok' : 'status-pill off'} title="Pronto para comprovante">
                    {c.hasPhone && c.hasEmail ? 'Contato OK' : 'Falta contato'}
                  </span>
                  <button className="ghost-btn" type="button" onClick={() => openEmployee(employee)}>
                    Abrir ficha
                  </button>
                  <button className="ghost-btn" type="button" onClick={() => void toggleActive(employee)}>
      ...
      <div className="card">
        {!selected ? (
          <div>
            ...
            <p className="small-muted">
              Clique em alguém na lista para ver e editar documentos, endereço e canais de contato (base para envio de comprovante).
            </p>
          </div>
        ) : (
          <>
            <div className="section-heading">
              <div>
                ...
                <p className="small-muted">
                  ...
                  <br />
                  <em style={{ opacity: 0.85 }}>Matrícula e jornada não são editáveis nesta tela (expediente em uso).</em>
                </p>
              </div>
            </div>
            ...
            <div className="form-two-col">
              <label className="small-muted">CPF<input className="input" value={cpfForm} onChange={(e) => setCpfForm(e.target.value)} placeholder="somente números" /></label>
            ...
            <div className="form-two-col">
              <label className="small-muted">Celular / WhatsApp<input className="input" value={profileForm.phone || ''} onChange={(e) => setField('phone', e.target.value)} placeholder="DDD + número" /></label>
              <label className="small-muted">E-mail pessoal<input className="input" type="email" value={profileForm.personalEmail || ''} onChange={(e) => setField('personalEmail', e.target.value)} placeholder="nome@email.com" /></label>
            </div>
            <p className="certificate-help">
              {comms.hasPhone && comms.hasEmail
                ? '✓ Celular e e-mail prontos para envio de comprovante.'
                : !comms.hasPhone && !comms.hasEmail
                  ? 'Informe celular e e-mail para habilitar comprovante por SMS e e-mail.'
                  : !comms.hasPhone
                    ? 'Falta celular para SMS.'
                    : 'Falta e-mail para envio de comprovante.'}
            </p>
            ...
            <div className="form-two-col">
              <label className="small-muted">RG / Identidade<input className="input" value={profileForm.rg || ''} onChange={(e) => setField('rg', e.target.value)} /></label>
            ...
            <div className="form-two-col">
              <label className="small-muted">Nascimento<input className="input" value={profileForm.birthDate || ''} onChange={(e) => setField('birthDate', e.target.value)} placeholder="DD/MM/AAAA" /></label>
              <label className="small-muted">Admissão<input className="input" value={profileForm.admissionDate || ''} onChange={(e) => setField('admissionDate', e.target.value)} placeholder="DD/MM/AAAA" /></label>
            </div>
            <div className="form-two-col">
              <label className="small-muted">Sexo<input className="input" value={profileForm.sex || ''} onChange={(e) => setField('sex', e.target.value)} placeholder="F / M" /></label>
              <label className="small-muted">Estado civil<input className="input" value={profileForm.maritalStatus || ''} onChange={(e) => setField('maritalStatus', e.target.value)} placeholder="C/S/D/O" /></label>
            </div>
            ...
            <div className="row-actions" style={{ marginTop: 16 }}>
              <button className="primary-btn" type="button" disabled={saving} onClick={() => void saveProfile()}>
