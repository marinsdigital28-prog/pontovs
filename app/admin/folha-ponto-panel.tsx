      ...
      <div className="section-heading">
        ...
        <div className="row-actions folha-print-actions">
          ...
          <select className="input folha-employee-select" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}>
            ...
            {employees.map((e) => (
              <option key={e.id} value={e.id}>{e.employeeNumber ? `${e.employeeNumber} · ` : ''}{e.name}</option>
            ))}
          </select>
          <button type="button" className="ghost-btn" onClick={() => void load()} disabled={loading}>Atualizar</button>
          <button type="button" className="ghost-btn" onClick={handlePrintAll}>Imprimir todos</button>
          <button type="button" className="primary-btn" onClick={() => void signAllPdfs()} disabled={signing}>{signing ? 'Gerando...' : 'PDF assinado (todos)'}</button>
        </div>
      </div>

      {batchProgress ? <p className="folha-batch-progress">{batchProgress}</p> : null}
      {error ? <p className="status-msg">{error}</p> : null}

      <div className="folha-competence-banner">
        <strong>{monthLabel(month)}</strong>
        <span>Competência <b>{month}</b></span>
        <span>{visibleEmployees.length} colaborador{visibleEmployees.length === 1 ? '' : 'es'}</span>
        <span className="folha-live-dot">ao vivo</span>
      </div>

      {preCloseAudit.blockers > 0 ? (
        <div className={`folha-preclose warn`}>
          <div className="folha-preclose-head">
            <strong>Pré-fechamento</strong>
            <span>{preCloseAudit.blockers} pendência(s)</span>
          </div>
          <ul className="folha-preclose-list">
            {preCloseAudit.noSchedule.length ? <li>Sem escala: {preCloseAudit.noSchedule.slice(0, 5).join('; ')}{preCloseAudit.noSchedule.length > 5 ? '...' : ''}</li> : null}
            {preCloseAudit.totalIncompletos ? <li>Incompletos: {preCloseAudit.totalIncompletos} dia(s)</li> : null}
            {preCloseAudit.totalFaltas ? <li>Faltas: {preCloseAudit.totalFaltas} dia(s)</li> : null}
            {preCloseAudit.pendingRequests.length ? <li>Solicitações pendentes: {preCloseAudit.pendingRequests.length}</li> : null}
            {preCloseAudit.pendingCerts.length ? <li>Atestados pendentes: {preCloseAudit.pendingCerts.length}</li> : null}
          </ul>
        </div>
      ) : (
        <div className="folha-preclose ok">
          <p className="folha-preclose-ok">✓ Pronto para fechamento — sem bloqueios na competência.</p>
        </div>
      )}

      {loading ? <p className="small-muted">Carregando marcações...</p> : null}
