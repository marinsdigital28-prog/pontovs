...
const certificateTypes = ['DIA_INTEGRAL','PERIODO_DIAS','HORAS','PERIODO_HORAS','CONSULTA_MEDICA','SAIDA_MEDICA','TRABALHO_EXTERNO','TRABALHO_EXTERNO_HORAS','OUTRO'] as const;
...
export async function POST(request: Request) {
  ...
  const candidates = await prisma.medicalCertificate.findMany({
    where: { userId: input.userId, status: { not: 'CANCELADO' }, startDate: { lte: endDate }, endDate: { gte: startDate } },
  ...
  if (overlap) {
    return NextResponse.json({
      error: hourly ? 'Existe sobreposição com outro lançamento neste horário.' : 'Já existe um abono/atestado cadastrado para parte deste período.',
    }, { status: 409 });
  }

  const created = await prisma.medicalCertificate.create({
    data: {
      userId: input.userId, createdById: session.user.id, type: input.type, startDate, endDate,
      startTime: hourly ? input.startTime : null, endTime: hourly ? input.endTime : null,
      hoursPerDayMinutes: hourly ? hoursPerDayMinutes : null,
      daysCount: hourly ? 0 : inclusiveDays(startDate, endDate),
      documentName: input.documentName || null, documentMime: input.documentMime || null,
      documentData: input.documentData || null, observation: input.observation || null, status: 'PENDENTE',
    },
    select: { id: true, type: true, startDate: true, endDate: true, startTime: true, endTime: true, hoursPerDayMinutes: true, status: true, daysCount: true },
  });
  await appendAuditEvent({
    action: 'ATESTADO_CRIADO', actorId: session.user.id, resource: 'MedicalCertificate', resourceId: created.id,
    metadata: { userId: input.userId, type: input.type, startDate: input.startDate, endDate: input.endDate, startTime: input.startTime || null, endTime: input.endTime || null, hoursPerDayMinutes, daysCount: created.daysCount, status: 'PENDENTE' },
  });
...
export async function PATCH(request: Request) {
  ...
  const existing = await prisma.medicalCertificate.findUnique({
    where: { id: body.id },
    select: { id: true, userId: true, status: true, startDate: true, endDate: true, startTime: true, endTime: true, type: true },
  });
  ...
  if (body.action === 'approve' || body.action === 'reject') {
    ...
    const updated = await prisma.medicalCertificate.update({
      where: { id: body.id }, data: { status: nextStatus },
      select: { id: true, status: true },
    });
    let punchesRejected = 0;
    if (body.action === 'approve') {
      const rangeStart = new Date(existing.startDate); rangeStart.setUTCHours(0, 0, 0, 0);
      const rangeEnd = new Date(existing.endDate); rangeEnd.setUTCHours(23, 59, 59, 999);
      const punches = await prisma.punch.findMany({
        where: { userId: existing.userId, status: 'VALID', timestamp: { gte: rangeStart, lte: rangeEnd } },
        select: { id: true, userId: true, type: true, timestamp: true, status: true },
      });
      const pairs = findConflictingPunches(punches, [{
        id: updated.id, userId: existing.userId, type: existing.type,
        startDate: existing.startDate, endDate: existing.endDate,
        startTime: existing.startTime, endTime: existing.endTime, status: 'APROVADO',
      }]);
      for (const { punch } of pairs) {
        const window = existing.startTime && existing.endTime ? `${existing.startTime}–${existing.endTime}` : 'dia integral';
        await prisma.$transaction(async (tx) => {
          await tx.punch.update({ where: { id: punch.id }, data: { status: 'REJECTED', origin: 'ADJUSTED' } });
          await tx.punchAudit.create({
            data: {
              id: crypto.randomUUID(), punchId: punch.id, changedById: session.user.id,
              field: 'status', oldValue: 'VALID', newValue: 'REJECTED',
              reason: `Conflito com atestado/trabalho externo aprovado (${window}): não é possível estar em dois lugares ao mesmo tempo`,
            },
          });
        });
        punchesRejected += 1;
    ...
    await appendAuditEvent({
      ...
      actorId: session.user.id, resource: 'MedicalCertificate', resourceId: existing.id,
      metadata: { userId: existing.userId, reason: body.reason?.trim() || null, punchesRejected },
    });
    return NextResponse.json({ certificate: updated, punchesRejected });
  }
  if (body.action === 'cancel') {
    if (existing.status === 'CANCELADO') return NextResponse.json({ error: 'Atestado já cancelado.' }, { status: 409 });
    const updated = await prisma.medicalCertificate.update({
      where: { id: body.id },
      data: { status: 'CANCELADO', canceledAt: new Date(), canceledById: session.user.id, cancelReason: body.reason?.trim() || 'Cancelamento administrativo' },
  ...
  const updated = await prisma.medicalCertificate.update({
    where: { id: body.id }, data: { observation: body.observation?.trim() || null },
