...
function parseProfile(profileJson: string | null | undefined) {
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    employees: employees.map((e) => ({
      ...e,
      profile: parseProfile(e.profileJson),
  ...
  try {
    const employee = await prisma.user.create({
      data: {
        ...
        name,
        employeeNumber,
        cpf: body?.cpf ? normalizeCpf(String(body.cpf)) : null,
        jobTitle: body?.jobTitle ? String(body.jobTitle).trim() : null,
        workDays: body?.workDays ? String(body.workDays).trim().toUpperCase() : null,
        scheduleStart: body?.scheduleStart ? String(body.scheduleStart) : null,
        scheduleEnd: body?.scheduleEnd ? String(body.scheduleEnd) : null,
        scheduleByDay: body?.scheduleByDay ? JSON.stringify(body.scheduleByDay) : null,
        profileJson: body?.profile ? JSON.stringify(body.profile) : null,
        email: `employee-${employeeNumber}@local.invalid`,
        role: 'EMPLOYEE',
        active: body?.active !== false,
      },
      select: { id: true, name: true, employeeNumber: true, cpf: true, jobTitle: true, workDays: true, scheduleStart: true, scheduleEnd: true, scheduleByDay: true, profileJson: true, active: true },
    });
    ...
    return NextResponse.json({ employee: { ...employee, profile: parseProfile(employee.profileJson), profileJson: undefined } }, { status: 201 });
  } catch (error: any) {
...
export async function PATCH(request: Request) {
  ...
  const current = await prisma.user.findFirst({
    ...
    select: { name: true, employeeNumber: true, cpf: true, jobTitle: true, workDays: true, scheduleStart: true, scheduleEnd: true, scheduleByDay: true, profileJson: true },
  });
  ...
  const candidate = {
    ...
    employeeNumber: body.employeeNumber !== undefined ? String(body.employeeNumber) : current.employeeNumber || '',
    cpf: body.cpf !== undefined ? String(body.cpf) : current.cpf || '',
    jobTitle: body.jobTitle !== undefined ? String(body.jobTitle) : current.jobTitle || '',
    workDays: body.workDays !== undefined ? String(body.workDays) : current.workDays || '',
    scheduleStart: body.scheduleStart !== undefined ? String(body.scheduleStart) : current.scheduleStart || '',
    scheduleEnd: body.scheduleEnd !== undefined ? String(body.scheduleEnd) : current.scheduleEnd || '',
  };
  ...
  for (const key of ['cpf', 'jobTitle', 'workDays', 'scheduleStart', 'scheduleEnd', 'scheduleByDay'] as const) {
    if (body[key] !== undefined) {
      data[key] = body[key] ? (key === 'cpf' ? normalizeCpf(String(body[key])) : String(body[key]).trim()) : null;
    }
  }
  if (body.profile !== undefined) {
    if (body.profile === null) data.profileJson = null;
    else if (typeof body.profile === 'object') {
      let prev: Record<string, unknown> = {};
      try {
        prev = current.profileJson ? JSON.parse(current.profileJson) : {};
      } catch {
        prev = {};
      }
      data.profileJson = JSON.stringify({ ...prev, ...body.profile });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ employee: { ...employee, profile: parseProfile(employee.profileJson), profileJson: undefined } });
  } catch (error: any) {
    if (error?.code === 'P2002') return NextResponse.json({ error: 'A matrícula ou CPF já está cadastrado.' }, { status: 409 });
