  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const employee of employees) {
        ...
        const user = existing
          ? await tx.user.update({ where: { employeeNumber }, data: { name, jobTitle, active: true }, select: { id: true } })
          : await tx.user.create({ data: { id: crypto.randomUUID(), name, employeeNumber, email: `${employeeNumber}@employee.local`, role: 'EMPLOYEE', active: true, jobTitle }, select: { id: true } });
      ...
      for (const punch of punches) {
        ...
        const type = String(punch.type ?? '').trim().toUpperCase();
        const date = String(punch.date ?? '').trim();
        const time = String(punch.time ?? '').trim();
        if (!userId || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}:\d{2}$/.test(time) || !allowedTypes.has(type)) { rowsIgnored += 1; continue; }
      ...
      return { employeesUpdated, punchesCreated, punchesExisting, rowsIgnored };
    }, { timeout: 120_000 });
