...
type EmployeeRequest = { id: string; type: 'AUSENCIA' | 'TROCA_DIA'; status: 'PENDENTE' | 'APROVADO' | 'REJEITADO' | 'CANCELADO'; startDate: string; endDate: string; reason: string; details?: string | null; reviewNote?: string | null; createdAt: string };
...
export default function ColaboradorPage() {
  ...
  async function loadRequests() { const response = await fetch('/api/employee/requests', { cache: 'no-store' }); if (response.ok) setRequests((await response.json()).requests || []); }

  useEffect(() => { if (sessionStatus === 'authenticated') { void loadHistory(); void loadRequests(); } }, [sessionStatus]);

  async function login(event: FormEvent) {
    event.preventDefault(); setLoading(true); setError('');
    const result = await signIn('credentials', { employeeNumber: normalizeEmployeeNumber(employeeNumber), redirect: false });
  ...
  async function submitRequest(event: FormEvent<HTMLFormElement>, label: string) { event.preventDefault(); setLoading(true); setError(''); const form = new FormData(event.currentTarget); const isAbsence = label.includes('ausência'); const payload = { type: isAbsence ? 'AUSENCIA' : 'TROCA_DIA', startDate: String(form.get(isAbsence ? 'date' : 'from') || ''), endDate: String(form.get(isAbsence ? 'date' : 'to') || ''), reason: String(form.get('reason') || ''), details: String(form.get('note') || '') || null }; try { const response = await fetch('/api/employee/requests', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || 'Não foi possível enviar a solicitação.'); setNotice(`${label} enviada para aprovação do gestor.`); await loadRequests(); setActivePanel('overview'); } catch (err) { setError(err instanceof Error ? err.message : 'Falha ao enviar solicitação.'); } finally { setLoading(false); } }
